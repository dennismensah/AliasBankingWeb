...
import { throwToolbarMixedModesError } from '../../../../../node_modules/@angular/material';
...
@Component({
  ...
  styleUrls: ['./iedit.component.scss']
})
export class IEditComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  constructor(private formBuilder: FormBuilder, private ratingService: RatingService,
    ...
    private fileService: FileService, private router: Router, private spinnerService: Ng4LoadingSpinnerService) { }
  ...
  ngOnInit() {
    this.route.data.subscribe(
      (data: Data) => {
        ...
        // alert(data['individual'])
      }
    );
    // const individualid = +this.route.snapshot.paramMap.get('id');
    // this.individualService.getIndividual(individualid).subscribe(res => this.Individual = res);
  ...
  onSubmit() {
    ...
    this.type.patchValue({
       'IndividualCustId': this.Individual['IndividualCustId'],
       'CustomerNumber': this.Individual['CustomerNumber'],
       'CreatedUserId': this.Individual['CreatedUserId'],
      'CreatedDate': this.Individual['CreatedDate']
    });
    this.individualService.updateIndividual(this.Individual['IndividualCustId'], this.type.value).subscribe(
      res => {
        this.spinnerService.hide();
        Alerts.showSuccessSweetAlert('Update Successfull', 'Individual has been updated');
